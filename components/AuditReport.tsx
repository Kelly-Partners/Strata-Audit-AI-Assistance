...
// Azure: PDF URL resolution is provided via getPdfUrlFn prop from App.tsx
import {
  AuditResponse,
  TraceableValue,
  DocumentEntry,
        ...
        >
           ...
           <div className="p-5 space-y-4 max-h-[400px] overflow-y-auto custom-scrollbar">
              ...
              <div>
                ...
                <div className="text-body font-bold text-black bg-gray-50 border border-gray-200 p-3 break-words leading-snug">
                  {doc ? (
                    <span>
              ...
              <div className="grid grid-cols-2 gap-4">
                 <div>
                    <span className="text-caption uppercase text-gray-400 font-bold block mb-1 tracking-widest">Doc ID</span>
                    <div className="text-body text-gray-600 bg-gray-100 px-2 py-1.5 border border-gray-200">
                      {val.source_doc_id}
                    </div>
                 </div>
                 <div>
                    <span className="text-caption uppercase text-gray-400 font-bold block mb-1 tracking-widest">Extracted</span>
                    <div className="text-body text-black bg-yellow-50 px-2 py-1.5 border border-yellow-100 font-bold">
                      {displayVal}
                    </div>
                 </div>
              </div>
              ...
              {val.verbatim_quote && (
                <div>
                   <span className="text-caption uppercase text-gray-400 font-bold block mb-1 tracking-widest flex items-center gap-1">
              ...
              {val.computation && (
                 <div>
                    ...
                    <div className="bg-blue-50 border border-blue-100 p-2 text-caption text-blue-900 font-mono">
              ...
              {val.note && (
                <div>
                   <span className="text-caption uppercase text-gray-400 font-bold block mb-1 tracking-widest">Context / Note</span>
                   <div className="text-body text-gray-600 italic">
                      {val.note}
                   </div>
                </div>
              )}
      ...
      <div className="fixed inset-0 z-[10000] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
        <div className="bg-white w-full max-w-md rounded shadow-2xl border-t-4 border-[#004F9F] overflow-hidden" onClick={e => e.stopPropagation()}>
          ...
          <div className="p-5 space-y-4 max-h-[400px] overflow-y-auto">
            ...
            <div>
              <span className="text-caption uppercase text-gray-400 font-bold block mb-1 tracking-widest">Doc ID</span>
              <div className="text-body text-gray-600 bg-gray-100 px-2 py-1.5 border border-gray-200">{payload.source_doc_id || '–'}</div>
            </div>
            ...
            <div>
              <span className="text-caption uppercase text-gray-400 font-bold block mb-1 tracking-widest">Context / Note</span>
              <div className="text-body text-gray-600 italic">{payload.note || '–'}</div>
            </div>
            <div className="pt-2">
              <button
                onClick={handleOpenPdf}
            ...
            <div className="p-6 space-y-4">
               {steps.map((step, idx) => (
                  <div key={idx} className="flex gap-4 border-b border-gray-100 pb-4 last:border-0 last:pb-0">
                     <div className="mt-1">
                        {step.status === 'PASS' 
                           ? <div className="w-6 h-6 rounded-full bg-green-100 text-green-700 flex items-center justify-center text-caption font-bold">✓</div>
                           : <div className="w-6 h-6 rounded-full bg-red-100 text-red-700 flex items-center justify-center text-caption font-bold">✕</div>
                        }
                     </div>
                     <div className="flex-1">
                        <div className="text-body font-bold text-gray-800">{step.rule}</div>
                        <div className="text-caption text-gray-500 italic mt-1">Ref: {step.evidence_ref}</div>
                     </div>
                  </div>
               ))}
            </div>
            <div className="bg-gray-50 px-6 py-4 text-center">
               <button onClick={onClose} className="text-caption font-bold uppercase tracking-widest text-gray-500 hover:text-black">Close Verification</button>
            </div>
         </div>
      </div>
   );
   ...
   return (
      <div className="fixed inset-0 z-[10000] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in" onClick={onClose}>
         <div className="bg-white w-full max-w-md rounded shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between items-center">
               <h3 className="font-bold text-body text-black uppercase tracking-wide">
            ...
            <div className="p-6">
               <div className="mb-4">
                  <span className="text-caption font-bold text-gray-400 uppercase tracking-widest">Item</span>
                  <div className="font-bold text-gray-800 text-body mt-1">{title}</div>
               </div>
               
               <div className="mb-4">
                  <label className="block text-caption font-bold text-gray-500 uppercase tracking-widest mb-2">Severity</label>
                  <div className="flex gap-2">
                     {(['low', 'medium', 'critical'] as const).map(s => (
                        <button 
                           key={s}
                           onClick={() => setSeverity(s)}
                           className={`px-3 py-1.5 text-caption font-bold uppercase tracking-wider rounded border ${
                              severity === s 
...
export const AuditReport: React.FC<AuditReportProps> = ({
  ...
  additionalRuns = [], onAddEvidenceClick, filePaths = [], getPdfUrlFn,
}) => {
      ...
      <div className="p-10 bg-[#FAFAFA] min-h-[500px]">
            ...
            <div className="bg-white p-8 rounded border border-gray-200 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
                ...
                {summary.missing_critical_types && summary.missing_critical_types.length > 0 ? (
                   ...
                   <div className="text-heading-sm font-medium text-red-900">
                      ...
                      [...docs].sort((a, b) => (a.Document_Type || '').localeCompare(b.Document_Type || '') || (a.Document_ID || '').localeCompare(b.Document_ID || '')).map((doc) => (
                        <tr key={doc.Document_ID} className="bg-white hover:bg-gray-50 transition-colors group">
                          ...
                          <td className="px-5 py-4 border-r border-gray-100">
                            <span className={`px-2 py-1 text-caption font-bold uppercase tracking-wider ${
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                      ...
                      if (!loc || typeof loc !== 'object' || Array.isArray(loc)) {
                        return (
                          <div key={key} className="p-4 bg-gray-50 border border-gray-100 rounded">
                            <div className="text-label text-gray-500 uppercase font-bold tracking-widest mb-1">{label}</div>
                            <div className="text-body font-mono text-gray-800">–</div>
                          </div>
                        );
                      }
                      ...
                      return (
                        ...
                        >
                          <div className="text-label text-gray-500 uppercase font-bold tracking-widest mb-1">
                            {label}
                          </div>
                          <div className="text-body text-gray-800 break-words">
              ...
              <div className="mt-10 pt-8 border-t border-gray-200">
                ...
                {safeData.bs_extract && Array.isArray(safeData.bs_extract.rows) && safeData.bs_extract.rows.length > 0 ? (
                  <>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      ...
                      <div className="p-3 bg-gray-50 border border-gray-200 rounded">
                        ...
                        <div className="text-body font-bold font-mono">{safeData.bs_extract.prior_year_label || '–'}</div>
                      </div>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="min-w-full text-left border border-gray-200">
                  ...
                  <div className="p-4 bg-amber-50 border border-amber-200 rounded text-amber-800 text-body">
          ...
          <div className="space-y-8">
            <div className="bg-white p-8 rounded border border-gray-200 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-4 mb-6 bg-gray-50 p-4 border border-gray-100">
                  <div className="text-body">Source Doc: <span className="bg-white border border-gray-200 px-2 py-1 text-black font-bold text-heading-sm">{data.levy_reconciliation.master_table.Source_Doc_ID}</span></div>
                  <div className="text-body">AGM Date: <span className="font-bold text-heading-sm">{data.levy_reconciliation.master_table.AGM_Date}</span></div>
              </div>
                ...
                <table className="w-full table-fixed text-right border-collapse border border-gray-200 [&_td:nth-child(5)]:truncate [&_td:nth-child(1)]:sticky [&_td:nth-child(1)]:left-0 [&_td:nth-child(1)]:z-10 [&_td:nth-child(1)]:bg-white [&_td:nth-child(5)]:sticky [&_td:nth-child(5)]:right-0 [&_td:nth-child(5)]:z-10 [&_td:nth-child(5)]:bg-white">
                  ...
                  <tbody className="divide-y divide-gray-100 text-heading-sm">
                    ...
                    <tr className="bg-gray-50/50">
                        <td className="px-5 py-3 text-left font-bold text-gray-800 uppercase text-body tracking-wide">Prior Year (Opening)</td>
                        <td colSpan={4}></td>
                    </tr>
                    ...
                    <tr className="border-b border-gray-100 group hover:bg-gray-50">
                     ...
                     {/* LEVIES STRUCK */}
                    <tr className="bg-gray-50/50">
                        <td className="px-5 py-3 text-left font-bold text-gray-800 uppercase text-body tracking-wide">Levies Struck (Net)</td>
                        <td colSpan={4}></td>
                    </tr>
                    <tr className="group hover:bg-gray-50">
                    ...
                    <tr className="bg-gray-50/50">
                        <td className="px-5 py-3 text-left font-bold text-gray-800 uppercase text-body tracking-wide">Reconciliation</td>
                        <td colSpan={4}></td>
                    </tr>
                    <tr className="group hover:bg-gray-50">
        ...
        {activeTab === 'levy' && !data.levy_reconciliation?.master_table && (
          <div className="bg-white p-8 rounded border border-gray-200 shadow-sm text-center py-16 text-gray-500">
            <p className="font-bold text-body">No Levy Reconciliation data</p>
            <p className="text-caption mt-1">Run Call 2 to populate Table E.Master</p>
          </div>
        )}
                                                ...
                                                <button type="button" onClick={() => setExpenseForensic({ pillar: 'FUND', rowIndex: idx, item })} className="text-left w-full border-b border-dotted border-[#004F9F] hover:bg-[#004F9F]/10 cursor-pointer px-2 py-1 rounded-sm" title="Click for Forensic Trace">
                                                    {fund ? (
                                                        <>
                                                            ...
                                                            {isMisclassified && <div className="text-label text-gray-600 mt-1">GL: {fund.gl_fund_code} | Inv: {fund.invoice_nature}</div>}
                                                        </>
                                                    ) : '–'}
                                                </button>
                                            </td>
                                ...
                                <tr key={idx} className="hover:bg-gray-50 align-top transition-colors group">
                                    ...
                                    <td className="px-5 py-4 border-r border-gray-100">
                                        <div className="flex flex-col gap-2">
                                            ...
                                            <div className="text-caption text-gray-400">{item.Source_Docs?.Invoice_ID ?? '–'}</div>
                                        </div>
                                    </td>
                                    <td className="px-5 py-4 border-r border-gray-100">
                                         <div className="flex flex-col gap-1">
                                            <div className="text-body text-gray-500">Code: <span className="text-black font-medium">{item.GL_Fund_Code ?? '–'}</span></div>
                                            <div className="text-body italic text-gray-600 mb-1">"{item.Inv_Desc ?? ''}"</div>
        ...
        {data.statutory_compliance?.gst_reconciliation && (
            <div className="bg-white p-8 rounded border border-gray-200 shadow-sm">
                {summary.registered_for_gst === false && (
                    <div className="mb-4 py-2 px-4 rounded bg-amber-50 border border-amber-200 text-amber-800 text-body font-medium">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-heading-sm">
                        <thead className="bg-gray-100 text-black uppercase font-bold text-body tracking-wider">
                            <tr>
                        ...
                        <tbody className="divide-y divide-gray-100">
                            ...
                            {/* BAS Activity */}
                            <tr><td colSpan={2} className="py-2 text-caption font-bold text-gray-400 uppercase tracking-widest pl-5">BAS Activity</td></tr>
                            {['Q1', 'Q2', 'Q3', 'Q4'].map((q) => {
                                // @ts-ignore
                 ...
                 <div className="bg-white p-8 rounded border border-gray-200 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="flex flex-col justify-center items-center p-6 bg-gray-50 border border-gray-200 relative group">
                             <span className="text-body text-gray-500 uppercase font-bold mb-3 tracking-widest">Overall Status</span>
        ...
        {!data.statutory_compliance?.gst_reconciliation && !data.statutory_compliance?.insurance && !data.statutory_compliance?.income_tax && (
            <div className="py-12 text-center border-2 border-dashed border-gray-200 rounded-lg bg-gray-50">
                ...
                <p className="text-body text-gray-500 max-w-md mx-auto">Run a full audit to populate GST reconciliation, Insurance, and Income Tax.</p>
            </div>
        )}
