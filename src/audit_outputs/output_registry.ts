...
**MANDATORY – OLD RATE LEVIES / NEW RATE LEVIES (Phase 2 rules levy_old_new_levies_source, levy_old_new_rate, levy_financial_year):** Source ONLY from minutes. **PRIMARY (preferred):** Extract from minutes **specific levy amounts and payment/due dates**; using LOCKED intake_summary.financial_year, keep only payments whose **date falls within the audit FY**; sum by Old vs New per minutes → output Old_Levy_* and New_Levy_*. **FALLBACK:** If minutes do not provide payment-level amounts and dates, use adoption date from minutes and time-apportion within FY (e.g. quarters, proportion by days or months). For every Old_Levy_* and New_Levy_* fill "note" and, if calculated, "computation" stating which method (primary: amounts+dates in FY; or fallback: adoption date + proportion) and the basis. source_doc_id and page_ref must cite minutes only.
