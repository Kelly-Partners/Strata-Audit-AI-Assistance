...
**LEVY MASTER TABLE – SYMBOL MAP:** (A) = Net_Opening_Bal; (B1) = Sub_Levies_Standard_Admin / Sub_Levies_Standard_Sink / Sub_Levies_Standard; (B) = Sub_Admin_Net / Sub_Sink_Net / Total_Levies_Net; (C) = Total_GST_Raised; (D) = Total_Gross_Inc; (E) = Effective_Levy_Receipts; (=) = Calc_Closing.
...
- **(E) Effective_Levy_Receipts** = Total_Receipts_Global - Non_Levy_Income. **MANDATORY – Total_Receipts_Global (Phase 2 TOTAL RECEIPTS – Admin & Capital Actual Payments):** Actively find (1) Admin Fund receipt/payment summary for the audit FY and (2) Capital/Sinking Fund receipt/payment summary for the audit FY, from the whitelist (Levy Receipts Report, Levy Summary by Fund, Fund Ledger – Admin/Capital, Contribution Report, etc.). Total_Receipts_Global = Admin receipts + Capital receipts; Effective_Levy_Receipts = Total_Receipts_Global; Non_Levy_Income = 0 when using Admin & Capital Actual Payments. Fallback: single combined cash-based receipt summary with Admin+Capital; then Non_Levy_Income from same source if identified. If neither available → Not Resolved – Boundary Defined.
...
- **CRITICAL – bs_amount & line_item SOURCE:** line_item and bs_amount MUST be copied from the **Balance Sheet (Financial Statement)** ONLY. Do NOT use GL, Levy Report, ledger, or summary for bs_amount or line_item. supporting_amount = verification evidence (per R2–R5); bs_amount = what the BS says.
...
- For each line: { "line_item", "section", "fund", "bs_amount" (MUST be from Balance Sheet FS CURRENT YEAR column – NOT from GL/ledger/summary), "supporting_amount" (verification evidence per R2–R5), "evidence_ref" (Doc_ID/Page for traceability), "status", "note" }.
